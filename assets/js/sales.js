...
function rowSelected(id) {
    $.ajax({
        url: '/api/cafeteria/sales/get_sale/' + id,
        type: 'GET',
        // dataType: 'json',
        success: function (data) {
            ...
            $('#salesDate').val(data[0]['sale_date']);
            ...
            $('#salesStatus').val(data[0]['order_id']['order_status']);
            $('#sale-return-id').val(data[0]['id']);
            // $('#sale_items').empty();
        },
        error: function (data) {
            alert('Error');
...
function searchSales(value) {
    ...
    if (type == "Customer Name") {
        $.ajax({
            url: '/api/cafeteria/sales/search_sales_customer/',
            type: 'GET',
        ...
        $.ajax({
            url: '/api/cafeteria/sales/search_sales_invoiceID/' + value.value,
            type: 'GET',
            // dataType: 'json',
            success: function (data) {
                createTable(data);
...
function createTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        row += '<td class="p-2">'+
        '<span onclick="rowSelected(' + data[i]['id'] + ')" @click="openViewCafeteriaSalesModal">' +
        ...
        row += "<td class='p-2'>" + data[i]['sale_date'] + "</td>";
